...
export default function AnalysisPage() {
  const plannedRatio = Math.round(
    (mockAnalysis.plannedMinutes / mockAnalysis.actualMinutes) * 100,
  );
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-5 py-10 pb-28 md:px-10 md:py-14 lg:px-14">
        ...
        <section className="grid gap-5 md:grid-cols-2" aria-label="AI analiz kartları">
              ...
              <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
                <div>
                  <div className="mb-3 flex items-end justify-between gap-4 text-sm">
                    <p className="text-muted-foreground">
                      Planlanan: <strong className="text-foreground">{mockAnalysis.plannedMinutes} dk</strong>
                    </p>
                    <p className="text-right text-muted-foreground">
                      Gerçek: <strong className="text-foreground">{mockAnalysis.actualMinutes} dk</strong>
                    </p>
                  </div>
                  <div
                    className="flex h-4 overflow-hidden rounded-full bg-muted"
                    role="img"
                    aria-label={`${mockAnalysis.plannedMinutes} dakika planlandı, ${mockAnalysis.actualMinutes} dakika kullanıldı`}
                  >
                    <div
                      className="h-full bg-primary"
                      style={{ width: `${plannedRatio}%` }}
                    />
                    <div
                ...
                <div className="flex items-center gap-2 rounded-xl bg-secondary/55 px-4 py-3 text-secondary-foreground">
                  ...
                  <span className="text-sm font-semibold">
                    +{mockAnalysis.unintendedMinutes} dk niyet dışı kullanım
                  </span>
                </div>
              </div>
            </CardContent>
