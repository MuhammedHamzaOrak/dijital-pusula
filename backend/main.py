...
async def analyze_activity(data: İstekVerisi):
    # Frontend'den gelen 'previousActivity' ve 'mood' verilerini prompttaki 'Baglam' değişkeni için birleştiriyoruz
    baglam_birlestirilmis = f"Önceki aktivitesi '{data.previousActivity}' şeklindeydi ve o anki ruh hali '{data.mood}' durumundaydı."
    
    # Promptun beklediği 5 değişkeni formatlıyoruz
    user_prompt = f"- Niyet: {data.intent}\n- Planlanan_Sure: {data.plannedMinutes} dakika\n- Gercek_Eylem: {data.actualActivity}\n- Gercek_Sure: {data.actualMinutes} dakika\n- Baglam: {baglam_birlestirilmis}"
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="AI servisine ulaşılamadı veya geçersiz yanıt alındı.")
